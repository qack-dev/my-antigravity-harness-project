...
- `AGENTS.md`と`GEMINI.md`(特にユーザーのホームディレクトリ配下`~/.gemini/GEMINI.md`にあるグローバルルール)が両方存在する場合の優先順位は、公式ドキュメントで明確な記載を確認できていない。コミュニティ記事では「競合時はGEMINI.mdが勝つ」という報告があるが、一次情報源で裏付けが取れていないため`[要確認]`として扱う。競合が心配な場合は、Antigravity固有の上書き設定は`~/.gemini/GEMINI.md`側に、プロジェクト固有かつクロスツールなルールは`AGENTS.md`側に書き分けること
